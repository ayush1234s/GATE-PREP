...
// Minimalist, high-end Vercel/Linear style sidebar navigation.
// Clean spacing, subtle left accent indicators, sleek dark mode aesthetic,
// and elegant user profile footer.
...
// ─── Single nav link (Sleek Linear/Vercel style) ──────────────────────────────
const SidebarLink = ({ to, icon: Icon, label, onClick }) => (
  <NavLink
    ...
    className={({ isActive }) =>
      `relative flex items-center gap-3.5 px-4 py-3 rounded-2xl text-sm font-semibold transition-all duration-200 group
       ${isActive
         ? 'bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 font-bold shadow-xs'
  ...
  >
    {({ isActive }) => (
      <>
        {/* Left glowing accent line for active tab */}
        {isActive && (
          <motion.div
            layoutId="sidebar-active-indicator"
            className="absolute left-0 top-2 bottom-2 w-1.5 rounded-r-full bg-primary-600 dark:bg-primary-400"
  ...
  return (
    <div className="relative flex-shrink-0">
      {photoURL ? (
        ...
        <div className={`${sz} rounded-xl bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white font-bold`}>
          {initials}
        </div>
      )}
      <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-400 border-2 border-white dark:border-card-dark" title="Active" />
    </div>
  )
}
...
const SidebarContent = ({ onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-card-dark text-slate-800 dark:text-white border-r border-slate-100 dark:border-slate-800/80">

      {/* ── 1. Clean Logo Header ── */}
      <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800/80">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-primary-600 via-primary-500 to-purple-600 flex items-center justify-center shadow-md shadow-primary-500/20">
            <GraduationCap className="w-5 h-5 text-white" />
          </div>
          <div>
            <p className="text-base font-extrabold text-slate-900 dark:text-white tracking-tight leading-none">
              GATE<span className="text-primary-600 dark:text-primary-400">-PREP</span>
            </p>
            <p className="text-[10px] text-slate-400 font-semibold mt-1 uppercase tracking-widest">ECE 2026 Edition</p>
          </div>
        </div>
        {/* Mobile close button */}
        {onClose && (
      ...
      <div className="px-4 py-3 mx-4 mb-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/60">
        ...
        <div className="h-1.5 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
          <div className="h-full bg-gradient-to-r from-primary-500 to-purple-500 rounded-full" style={{ width: `${progressPct}%` }} />
        </div>
      </div>

      {/* ── 4. Elegant User Footer ── */}
      <div className="p-4 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/40 dark:bg-slate-800/20">
