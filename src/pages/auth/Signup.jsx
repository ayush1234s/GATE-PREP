...
// Features a dynamic entrance animation: Student boy appears, opens study backpack,
// popping books, trophies, and light rays, revealing the signup form on the right. Also includes Admin Portal quick link.

import { useState }       from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { motion }         from 'framer-motion'
import { Eye, EyeOff, Mail, Lock, User, UserPlus, Loader2, Sparkles, Shield } from 'lucide-react'
import toast              from 'react-hot-toast'
import AuthLayout         from '@/layouts/AuthLayout'
import BoyBackpackAnimation from '@/components/auth/BoyBackpackAnimation'
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const profile = await initializeNewUser(user.uid, formData.name, user.email)

      try {
        await sendWelcomeEmail(user.email, formData.name, profile.studentId)
      } catch (err) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      const profile = await initializeNewUser(user.uid, user.displayName, user.email)

      try {
        await sendWelcomeEmail(user.email, user.displayName, profile.studentId)
      } catch (err) {
        console.warn('Welcome email notice:', err)
      }

      toast.success('Welcome to GATE-PREP! 🎉')
      navigate('/dashboard', { replace: true })
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div className="lg:hidden mb-4 p-3 rounded-2xl bg-gradient-to-r from-emerald-400/20 via-primary-500/20 to-indigo-500/20 border border-emerald-500/30 flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-800 dark:text-white">Student Backpack Ready!</h4>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">Fill in your details to open your study portal.</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Join thousands of GATE ECE aspirants building top AIR ranks.
          </p>
        </div>
