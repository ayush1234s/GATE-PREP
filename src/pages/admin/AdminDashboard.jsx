  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-900 via-indigo-950 to-slate-950 p-6 md:p-8 text-white shadow-2xl border border-purple-800/40"
      >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <motion.div whileHover={{ y: -3 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4 flex flex-col justify-between">
      ...
      {/* ── 4. Recent Registered Students Snapshot ── */}
      <motion.div variants={itemVariants} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 text-purple-400" />
            <h2 className="font-bold text-base text-white">Recent Registered Students</h2>
          </div>
          <button
            onClick={() => navigate('/admin/users')}
        ...
        {users.length === 0 ? (
          <p className="text-xs text-slate-500 py-4 text-center">No registered students found yet.</p>
        ) : (
            ...
            {users.slice(0, 6).map((u) => (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-xl bg-purple-600/30 text-purple-300 font-bold flex items-center justify-center text-xs flex-shrink-0">
                    {(u.name || u.email || 'S').slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-white truncate">{u.name || 'Student'}</p>
                    <p className="text-[11px] text-slate-400 truncate">{u.email}</p>
                  </div>
                </div>
                {u.disabled ? (
                  <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-red-900/40 text-red-400 border border-red-800/50 flex-shrink-0">
                    Disabled
                  </span>
                ) : (
                  <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-emerald-900/40 text-emerald-400 border border-emerald-800/50 flex-shrink-0">
                    Active
                  </span>
                )}
              </div>
